#!/usr/bin/env python3
"""
Telangana SCERT - English Medium Only Downloader
Downloads only English medium textbooks
"""
...
def main():
    print("=" * 80)
    print("TELANGANA STATE BOARD - ENGLISH MEDIUM ONLY DOWNLOAD")
    print("=" * 80)
    ...
    print(f"📚 Downloading: English medium textbooks only")
    print(f"📊 Total expected: 32 textbooks (~600-700 MB)")
    ...
    for grade_key in sorted(catalog['grades'].keys(), key=int):
        ...
        # Check if grade has English textbooks
        has_english = any('english' in mediums for mediums in grade_data['subjects'].values())
        if not has_english:
            continue
        ...
        for subject_name, subject_data in grade_data['subjects'].items():
            # Only download English medium
            if 'english' not in subject_data:
                continue

            textbook_info = subject_data['english']
    ...
    print(f"\n✅ Total English textbooks: {total_downloaded}/32")

    if stats['failed'] == 0:
        print("\n🎉 All English medium textbooks downloaded successfully!")
