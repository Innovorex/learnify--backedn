...
class CurriculumDataService:
    ...
    def _normalize_grade(self, grade: str) -> int:
        """Convert grade string to number, handling ranges like '6-8' by taking first grade"""
        grade_str = str(grade).lower().strip()

        # Handle grade ranges (e.g., "6-8", "1-5")
        if '-' in grade_str:
            first_grade = grade_str.split('-')[0].strip()
            return self._normalize_grade(first_grade)  # Recursively normalize first grade

        # Handle comma-separated grades (e.g., "6,7,8")
        if ',' in grade_str:
            first_grade = grade_str.split(',')[0].strip()
            return self._normalize_grade(first_grade)  # Recursively normalize first grade
    ...
    def get_cbse_computer_curriculum(self, grade: str) -> Dict[str, Any]:
        """Get CBSE Computer Science curriculum data for specific grade"""
        grade_num = self._normalize_grade(grade)
            ...
            return {
                "units": {
                    "Introduction to Computers": {
                        "topics": ["Parts of Computer", "Input/Output Devices", "Uses of Computer"]
                    },
                    "Operating System Basics": {
                        "topics": ["What is OS", "Desktop", "Files and Folders"]
                    },
                    "Basic Applications": {
                        "topics": ["Paint", "Text Editor", "Simple Games"]
                    }
                }
            }
        elif grade_num >= 6:
            return {
                "units": {
                    "Computer Fundamentals": {
                        "topics": ["Hardware", "Software", "Memory", "Storage"]
                    },
                    "Programming Concepts": {
                        "topics": ["Algorithms", "Flowcharts", "Basic Programming"]
                    },
                    "Internet and Communication": {
                        "topics": ["Internet Basics", "Email", "Web Browsing"]
                    }
                }
            }
        return {}

    def get_cbse_english_curriculum(self, grade: str) -> Dict[str, Any]:
        """Get CBSE English curriculum data for specific grade"""
        return {
            "units": {
                "Reading Comprehension": {
                    "topics": ["Stories", "Poems", "Information Texts"]
                },
                "Writing Skills": {
                    "topics": ["Essays", "Letters", "Creative Writing"]
                },
                "Grammar": {
    ...
    def get_general_curriculum(self, subject: str, grade: str) -> Dict[str, Any]:
        """Get general curriculum for any subject"""
        return {
            "units": {
                f"{subject} Fundamentals": {
                    "topics": [f"Basic {subject} concepts", f"{subject} applications", f"{subject} practice"]
                },
                f"Advanced {subject}": {
                    "topics": [f"Complex {subject} topics", f"{subject} problem solving"]
                }
            }
        }
    ...
    def get_curriculum_data(self, board: str, subject: str, grade: str) -> Dict[str, Any]:
        ...
        if board.upper() == "CBSE":
            if "math" in subject.lower():
                ...
                curriculum_data = self.get_cbse_computer_curriculum(grade)
            elif "english" in subject.lower():
                curriculum_data = self.get_cbse_english_curriculum(grade)
            else:
                curriculum_data = self.get_general_curriculum(subject, grade)
