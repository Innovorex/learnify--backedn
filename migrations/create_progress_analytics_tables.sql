-- ═══════════════════════════════════════════════════════════════
-- PROGRESS ANALYTICS - DATABASE SCHEMA
-- Migration: Create tables and materialized views for progress tracking
-- ═══════════════════════════════════════════════════════════════
...
CREATE INDEX idx_progress_teacher_date ON teacher_progress_snapshots(teacher_id, snapshot_date);
CREATE INDEX idx_progress_period ON teacher_progress_snapshots(period_type, snapshot_date);


-- ───────────────────────────────────────────────────────────────
-- 2. Materialized View: Current Teacher Scores
-- ───────────────────────────────────────────────────────────────
...
materials_scores AS (
    ...
    LEFT JOIN ai_tutor_sessions ats ON tm.id = ats.material_id
...
-- Refresh function (call this in a cron job)
...
-- Add to cron: 0 */6 * * * psql -d learnify -c "SELECT refresh_teacher_scores();"
