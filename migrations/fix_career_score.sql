-- Fix Career Score Calculation - Include Completed Courses
-- BUG: Previously only counted 'in_progress' courses, missing 'completed' courses
...
career_scores AS (
    ...
    WHERE tce.status IN ('in_progress', 'completed')  -- ✅ FIX: Include completed courses
...
SELECT
    ...
    u.email,
    -- Component Scores (explicitly cast to numeric)
    ROUND(CAST(COALESCE((COALESCE(cpd.mcq_avg, 0) * 0.40 + COALESCE(cpd.submission_avg, 0) * 0.35 + COALESCE(cpd.outcome_avg, 0) * 0.25), 0) AS NUMERIC), 1) as cpd_score,
    ROUND(CAST(COALESCE(((car.completed_modules * 1.0 / NULLIF(car.total_modules, 0)) * 70 + COALESCE(car.avg_exam_score, 0) * 0.30), 0) AS NUMERIC), 1) as career_score,
    ROUND(CAST(COALESCE(LEAST(100, COALESCE(stu.avg_student_score, 0) * 0.60 + (LEAST(stu.assessments_created, 20) / 20.0 * 100 * 0.40)), 0) AS NUMERIC), 1) as student_impact_score,
    ROUND(CAST(COALESCE((eng.active_days * 1.0 / 30 * 100), 0) AS NUMERIC), 1) as engagement_score,
    ROUND(CAST(COALESCE((LEAST(mat.materials_count, 15) / 15.0 * 100), 0) AS NUMERIC), 1) as materials_score,
    -- Overall Score
    ROUND(CAST(COALESCE(
    ...
    0) AS NUMERIC), 1) as overall_score,
    -- Supporting Metrics
...
-- Verify the fix
SELECT 'CAREER SCORE FIX SUCCESSFUL!' as status, COUNT(*) as teacher_count FROM teacher_current_scores;
SELECT teacher_name, overall_score, cpd_score, career_score, student_impact_score, completed_modules, total_modules
FROM teacher_current_scores
WHERE teacher_name = 'siva';
